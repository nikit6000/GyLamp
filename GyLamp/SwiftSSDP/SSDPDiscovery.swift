...
public class SSDPDiscovery: NSObject {
    ...
    func handleMessage(_ message: SSDPMessage) {
        ...
        if let searchTarget = responseSearchTarget {
            ...
            if searchTarget == .all {
                NKLog("Received MSEARCH response with ssdp:all")
                return;
            }
...
extension SSDPDiscovery: GCDAsyncUdpSocketDelegate {
    ...
    public func udpSocket(_ sock: GCDAsyncUdpSocket, didNotConnect error: Error?) {
        if (error != nil) {
            NKLog("Unable to connect \(String(describing: error))")
        } else {
            NKLog("Unable to connect")
        }
    }
    ...
    public func udpSocket(_ sock: GCDAsyncUdpSocket, didNotSendDataWithTag tag: Int, dueToError error: Error?) {
        if (error != nil) {
            NKLog("Unable to send data \(String(describing: error))")
        } else {
            NKLog("Unable to send data")
        }
    }
    
    public func udpSocket(_ sock: GCDAsyncUdpSocket, didReceive data: Data, fromAddress address: Data, withFilterContext filterContext: Any?) {
        //NKLog("M-SEARCH response handled")
        //NKLog(String(data: data, encoding: .utf8)!)
        
        // Ensure we have parsable data
        guard let messageString = String(data: data, encoding: .utf8) else {
            //NKLog("Unable to parse M-SEARCH response")
            return
        }
        ...
        guard let message = SSDPMessageParser.parse(response: messageString) else {
            //NKLog("incomplete M-SEARCH response\n\(messageString)")
            return
        }
